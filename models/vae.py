...
class Encoder(nn.Module):
    def __init__(self, input_channels, hidden_dims, latent_dim, dropout_rate=0.5):
        super(Encoder, self).__init__()
        # Inicializa uma lista para armazenar as camadas da rede
        modules = []
        self.dropout_rate = dropout_rate #ADICIONEI
        ...
        for h_dim in hidden_dims:
            modules.append(
                nn.Sequential(
                    ...
                    nn.ReLU(),
                    nn.Dropout(dropout_rate) #ADICIONEI (e a virg na linha de cima)
                )
            )
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, hidden_dims, output_channels, dropout_rate=0.5):
